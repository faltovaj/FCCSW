...
CombinedCaloTowerTool::CombinedCaloTowerTool(const std::string& type, const std::string& name, const IInterface* parent)
  ...
  declareProperty("ecalCells", m_ecalCells, "Ecal cells to create towers from (input)");
  declareProperty("hcalCells", m_hcalCells, "Hcal cells to create towers from (input)");
...
tower CombinedCaloTowerTool::towersNumber() {
 // maximum eta of the detector (== eta offset + half of the cell size)
  ...
  m_phiMax = std::max(fabs(m_ecalSegmentation->offsetPhi()) + M_PI / (double)m_ecalSegmentation->phiBins(),
		      fabs(m_hcalSegmentation->offsetEta()) + M_PI / (double)m_hcalSegmentation->phiBins());
...
uint CombinedCaloTowerTool::buildTowers(std::vector<std::vector<float>>& aTowers) {
  ...
  // Loop over a collection of ecal cells and build calo towers
  loopOverCells(aTowers, *ecalCells, *m_ecalSegmentation);
  // 2. HCAL
  // Get the input collection with cells from simulation + digitisation (after calibration and with noise)
  ...
  loopOverCells(aTowers, *hcalCells, *m_hcalSegmentation);
  return ecalCells->size() + hcalCells->size();
}

uint CombinedCaloTowerTool::loopOverCells(std::vector<std::vector<float>>& aTowers, const fcc::CaloHitCollection& aCells, DD4hep::DDSegmentation::GridPhiEta& aSegmentation) {
