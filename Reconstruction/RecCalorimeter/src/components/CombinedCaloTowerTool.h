...
public:
  ...
  /** Loop over cells in the collection and add the transverse energy to the appropriate tower(s)
   */
  uint loopOverCells(std::vector<std::vector<float>>& aTowers, const fcc::CaloHitCollection& aCells, DD4hep::DDSegmentation::GridPhiEta& aSegmentation);

private:
 /// Handle for electromagnetic calorimeter cells (input collection)
  DataHandle<fcc::CaloHitCollection> m_ecalCells{"calo/ecalCells", Gaudi::DataHandle::Reader, this};
  /// Handle for hadronic calorimeter cells (input collection)
  DataHandle<fcc::CaloHitCollection> m_hcalCells{"calo/hcalCells", Gaudi::DataHandle::Reader, this};
