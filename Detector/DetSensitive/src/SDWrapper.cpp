...
static G4VSensitiveDetector* create_full_particle_absorbtion_sd(const std::string& aDetectorName,
  ...
  // Factory method to create an instance of AllEnergyCalorimeterSD
static G4VSensitiveDetector* create_all_energy_calorimeter_sd(const std::string& aDetectorName,
							  dd4hep::Detector& aLcdd) {
  std::string readoutName = aLcdd.sensitiveDetector(aDetectorName).readout().name();
  return new det::AllEnergyCalorimeterSD(
      aDetectorName, readoutName, aLcdd.sensitiveDetector(aDetectorName).readout().segmentation());
}
}
}
...
DECLARE_EXTERNAL_GEANT4SENSITIVEDETECTOR(AllEnergyCalorimeterSD, dd4hep::sim::create_all_energy_calorimeter_sd)
